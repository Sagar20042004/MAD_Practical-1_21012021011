...
class Matrix(var data: Array<IntArray>) {
    ...
    operator fun times(other: Matrix): Matrix {
        val resultData = Array(rows) {IntArray(other.columns)
        {0} }
        for (i in 0 until rows) {
...
fun main() {
    ...
    val firstMatrix = Matrix(arrayOf(intArrayOf(3, -2, 5),
        intArrayOf(3, 0, 4)))
    val secondMatrix = Matrix(arrayOf(intArrayOf(2, 3),
        intArrayOf(-9, 0), intArrayOf(0, 4)))
    val secondMatrix1 = Matrix(arrayOf(intArrayOf(6, 3),
        intArrayOf(9, 0), intArrayOf(5, 4)))
    println("**** Addition ****")
